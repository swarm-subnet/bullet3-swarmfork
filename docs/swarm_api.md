...
- A new render flag takes the next free power of two; the next free value is 32768.
...
| `ER_TEXTURE_FILTER` | 16 | both, colour | Bilinear texture reads from a mip chain built once per texture on the first filtered frame; the level comes from the pixel footprint, from float exponent bits, not `log2`. The alpha plane is halved alongside; under `ER_SWARM_DAYLIGHT` with `ER_ALPHA_CUTOUT` a camera ray reads a cut-out over its pixel's footprint, and past two texels the surface is a veil, kept from 3 % coverage and lit by its coverage over the next surface along the ray, so thin wire or far leaf cards fade instead of breaking into moire; shadow rays keep the per-texel test | office 256 px, 1 thread: +2 ms per frame, +100 ms first frame, +41 MB for the office textures | [#9](https://github.com/swarm-subnet/bullet3-swarmfork/pull/9) |
...
| `ER_SWARM_LEAF_NO_SHADOW` | 16384 | ray cast, needs `shadow=1` | A double-sided surface with an alpha texture, a leaf or grass card, casts no shadow: the rays that build the shadow map and every occlusion ray pass through it, and its own shadow lookup is made from its sunward side, so the light through it (`ER_SWARM_DAYLIGHT`'s 35 %) is not cancelled by the map's rule that a face turned from the sun is in shadow. A crown of cards is lit as one crown instead of a stack of blinds; panels, walls and the ground keep every shadow, and the cards still receive them. Without the flag nothing changes | solar park, 960 x 540, 16 threads: the same frame time within noise; the shadow map is recast once when the flag changes | [#32](https://github.com/swarm-subnet/bullet3-swarmfork/pull/32) |

Dependencies between flags: `ER_SWARM_DAYLIGHT` acts only with `ER_SWARM_RAYCAST` and takes its sky from `ER_SWARM_SKY_SUN` or from `skyTextureId`; `ER_SWARM_SHADOW_MAP` acts only with `shadow=1` and `ER_SWARM_RAYCAST`; `ER_SWARM_MOVER_SHADOW` only with `ER_SWARM_SHADOW_MAP`; `ER_SWARM_LEAF_NO_SHADOW` only with `shadow=1` on the ray-cast path; `ER_EDGE_ANTIALIAS`, `ER_ALPHA_CUTOUT` and `ER_SWARM_LINEAR_LIGHT` only with `ER_SWARM_RAYCAST`; `ER_TEXTURE_FILTER`, `ER_SPECULAR_GLINT` and `ER_SWARM_SKY_SUN` work on both colour paths. `ER_DEPTH_ONLY` switches all colour work off whatever else is set.
...
| `VISUAL_SHAPE_RENDER_INSTANCED` | 128 | create; ray cast in `SwarmRaycast.cpp` | One local-space tree per mesh content hash, shared across body poses and mesh scales; the visual frame and scale live on the instance, normals use the inverse transpose, and material groups remain separate instances. These bodies cast into the shadow map without mover shadow rays; moving, hiding, retexturing or removing one recasts the whole map, so the flag is for bodies that stay put, never for movers. Takes precedence over the disk tree cache on that body; no disk files | 200 placements of one synthetic 60,000-triangle OBJ, 256 px, 4 threads, separate processes: peak RSS 2188.1 MB unflagged, 114.7 MB instanced (94.8 % less); five mesh scales share the same tree | [#34](https://github.com/swarm-subnet/bullet3-swarmfork/pull/34) |

A forest file (`.fst`) passed as `fileName` together with `VISUAL_SHAPE_RENDER_INSTANCED` places many copies of a few OBJ meshes as one body: `mesh <path>` lines, paths relative to the file, then one `<mesh index> x y z qx qy qz qw sx sy sz` line per placement, `#` for comments. Each mesh is converted once and all its placements become one Embree instance array under the body's pose, so a body that moves never re-sorts the trees and the frame cost follows the pixels, not the count. Measured: 60,000 placements of real-leaf crowns of about 60,000 triangles each, one body, 96-thread host, 4 render threads: build 1.6 s against 29 s as separate bodies, 256 px frame 66 ms still and 66 ms with a mover against 104 and 241 ms, 0.15 KB a placement. Without the flag the file draws nothing.
...
| `forestBatchTest.py` | `.fst` forest files: the same colour and depth as separate instanced bodies on the ray-cast, shadow map, daylight and raster paths, body pose, mask id, movers, the flag required, thread counts |
...
| `daylightTest.py` | `ER_SWARM_DAYLIGHT`: flag off unchanged, sun to sky ratio, sky light by direction, exposure, photo sky and yaw, haze, soft shadow, a shadow edge with no staircase, leaf light, a far cut-out as an even veil with near lines still sharp, thread counts |
| `leafNoShadowTest.py` | `ER_SWARM_LEAF_NO_SHADOW`: the ground under a leaf card is lit with the flag and shadowed without, an opaque box still casts, flag off unchanged, thread counts |
...
1. Take the next free bit of the enum it belongs to (render flags: 32768) and export the constant in `pybullet.c`.
